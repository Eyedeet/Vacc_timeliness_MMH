...
prod.files <- list.files(path = paste0(parquet, "filtered/"), pattern = "\\DrugIssue")
...
write_parquet(df, paste0(parquet, "filtered/DTP_outcome_uncleaned.parquet"))
df <- read_parquet(paste0(parquet, "filtered/DTP_outcome_uncleaned.parquet"))
...
length(unique(df$patid)) #1,696,896babies remaining
...
q1a  <- tmp[n_dose == 2 & age < 16]
length(unique(q1a$patid)) #4
dubious_q1 <- q1$patid
...
population <- read_parquet(paste0(datafiles, "study_population.parquet"))
pop_dub <- population[patid %chin% dubious_q1]
hist(pop_dub$yob) #interesting - decline from 2011
write_parquet(pop_dub, paste0(datafiles, "excluded_DTP_clean.parquet"))
...
age_gap_excl <- hist_diff12+hist_diff23+plot_layout(nrow=2)
ggsave("DTP_min_age_interval.pdf",
       age_gap_excl,
       width = 8,
       height = 6,
       bg = "white",
...
write_parquet(DTP, paste0(datafiles, "DTP_outcome.parquet"))
